...
function ICO() {
  ...
  const [metamaskDownloadEnabled, enableDownloadMetamask] =
  ...
  const tokenTransactionExecuter = async (
    ...
    if (coin_name !== 'BNB')
      return handleTokenPayment(coin_name, send_token_amount, quote);
  ...
  const handleAmountChange = (e) => {
    if (
      e.target.value *
        ((coinPrices[selectedPayment?.name] / coinPrices.RKL) *
          coinPrices.WIN) >
      6000
    ) {
      console.log(
  ...
  const setInitialTransaction = ({ to, send_token_amount }) => {
    const transaction = {
      to,
  ...
  const processTransaction = async (token, recipient, quote, transaction) => {
    // console.log(transaction);
    // transaction.wait().then(r => {console.log("Response", r)});
    console.log('Processesing');
    console.log(token, recipient, quote, transaction);
    try {
  ...
  const handleNativePayment = async (token, send_token_amount, quote) => {
    ...
    console.log(
      usersCurrentBalance / 1e18,
      ethers.utils.parseEther(`${send_token_amount}`).add(gasEstimate) / 1e18
    );
    if (
      usersCurrentBalance <=
      ethers.utils.parseEther(`${send_token_amount}`).add(gasEstimate)
    ) {
      console.log(
        'sendPayment for %s bnb and receive %s',
        ethers.utils
          .parseEther(`${send_token_amount}`)
          .add(gasEstimate)
          .toString(),
        quote
      );
      ...
      setError(
        `Insufficient funds! Aquire ${
          (usersCurrentBalance -
            ethers.utils.parseEther(`${send_token_amount}`).add(gasEstimate)) /
          1e18
        } more ${token} to cover gas for this transaction.`
      );
    }
    setIsLoadingPurchase(false);
  };
  ...
  const handleTokenPayment = async (token = 'RKL', amountIn, quote = 0) => {
    ...
    const walletSigner = getUserSigner(userProvider);
    const recipient = await walletSigner.getAddress();
    const tokenBSCContract = new ethers.Contract(
      tokens[token],
      tokenABI[token],
      provider
    );
    ...
    if (quote <= 6000) {
      try {
        const transactionHash = await contractSigner.transfer(
          FundraiserWallet,
          tokenAmountIn
        );
        return processTransaction(token, recipient, quote, transactionHash);
  ...
  const handleDirectSwap = async (amount, quote) => {
    ...
    const rickleBSCContract = new ethers.Contract(
      '0xeca15e1bbff172d545dd6325f3bae7b737906737',
      tokenABI['RKL'],
      walletSigner
    );
    const allowance = await rickleBSCContract.allowance(
      recipient,
      pancakeSwapContract.router
    );
    ...
    const amountsOut = await swapRouter.getAmountsOut(amountInEther, [
      tokens.RKL,
      tokens.WIN,
    ]); //usedBy swapExactTokensForTokens
    const amountsIn = await swapRouter.getAmountsIn(amountsOut[1].toString(), [
      tokens.RKL,
      tokens.WIN,
    ]); //usedBy swapTokensForExactTokens
  ...
  const handlePaymentReceipt = (
    ...
    axios
      .post('https://api.winston.services/ico/winston', {
        token,
        recipient,
        quote,
        transactionHash,
      ...
      .catch((error) => {
        setError(
          `An Error has happened, please contact support with the following transaction id.\n${transactionHash}`
        );
        setIsLoadingPurchase(false);
      });
  };
  ...
  const handleClick = () => {
    tokenTransactionExecuter(
      ...
      amount,
      amount *
        ((coinPrices[selectedPayment?.name] / coinPrices.RKL) * coinPrices.WIN)
    );
  };

  const handleSelectedPayment = (item) => {
    SetSelectedPayment(item);
    if (amount < 0) handlePriceImpactToHigh(1);
  };
  ...
  const loadDex = React.useCallback(async () => {
    const ahwaBSCContract = new ethers.Contract(
      tokens['AHWA'],
      tokenABI['AHWA'],
      provider
    );
    const winstonBSCContract = new ethers.Contract(
      tokens['WIN'],
      tokenABI['WIN'],
      provider
    );
    ...
    const fieldBSCContract = new ethers.Contract(
      tokens['FIELD'],
      tokenABI['FIELD'],
      provider
    );
    const wbnbBSCContract = new ethers.Contract(
      tokens['WBNB'],
      tokenABI['WBNB'],
      provider
    );
    const busdBSCContract = new ethers.Contract(
      tokens['BUSD'],
      tokenABI['WBNB'],
      provider
    );
    const wmueBSCContract = new ethers.Contract(
      tokens['WMUE'],
      tokenABI['WMUE'],
      provider
    );
    const usdcBSCContract = new ethers.Contract(
      tokens['USDC'],
      tokenABI['RKL'],
      provider
    );
    ...
    const winstonRKLYFBSCContract = new ethers.Contract(
      '0x42a3f09de1d0fa9ba589e4cdcf07be9d9890462c',
      winstonRKLYFABI,
      provider
    );
    const winstonRKLPSContract = new ethers.Contract(
      '0xbb22d416ac55e8d07d9fe78da3cc9919a1c357ce',
      winstonRKLPSABI,
      provider
    );
    const winstonAhwaPSContract = new ethers.Contract(
      '0x89d958a02ba82c55202af0cd6555bdd954de5595',
      winstonRKLPSABI,
      provider
    ); // ahwa res0
    ...
    const teamWallet = await winstonBSCContract.balanceOf(
      '0x130Be432f446F1fD0B356023f3b308C36a304642'
    );
    const burnedSupply = await winstonBSCContract.balanceOf(
    ...
    const winstonHeldInGasStation = await winstonBSCContract.balanceOf(
      GasStationWallet
    );
    const bnbHeldInGasStation = await provider.getBalance(GasStationWallet);

    const bnbHeldInFundraiserWallet = await provider.getBalance(
      FundraiserWallet
    );
    const wbnbHeldInFundraiserWallet = await wbnbBSCContract.balanceOf(
      FundraiserWallet
    );
    const busdHeldInFundraiserWallet = await busdBSCContract.balanceOf(
      FundraiserWallet
    );
    const rickleHeldInFundraiserWallet = await rickleBSCContract.balanceOf(
      FundraiserWallet
    );
    const fieldHeldInFundraiserWallet = await fieldBSCContract.balanceOf(
      FundraiserWallet
    );
    const wmueHeldInFundraiserWallet = await wmueBSCContract.balanceOf(
      FundraiserWallet
    );
    const usdcHeldInFundraiserWallet = await usdcBSCContract.balanceOf(
      FundraiserWallet
    );
    const usdtHeldInFundraiserWallet = await usdtBSCContract.balanceOf(
      FundraiserWallet
    );
    ...
    const psWINLp = {
      reserve0: heldInVoterPSLiquidity[0],
      reserve1: heldInVoterPSLiquidity[1],
    };
    const psRKLLp = {
      reserve0: heldInPSLiquidity[0],
      reserve1: heldInPSLiquidity[1],
    };
    const yfRKLLp = {
      reserve0: heldInYFLiquidity[0],
      reserve1: heldInYFLiquidity[1],
    };
    const psWBNBLpContract = new ethers.Contract(
      '0x4dce12240b37879610601eb70685d396faf06417',
      winstonRKLPSABI,
      provider
    );
    ...
    const psWBNBLp = {
      reserve0: psWBNBLpPSLiquidity[0],
      reserve1: psWBNBLpPSLiquidity[1],
    };
    const psBUSDLpContract = new ethers.Contract(
      '0x45d9c665055572970d246f2dd96c74787faef03f',
      winstonRKLPSABI,
      provider
    );
    const psBUSDLpPSLiquidity = await psBUSDLpContract.getReserves();
    const psBUSDLp = {
      reserve0: psBUSDLpPSLiquidity[0],
      reserve1: psBUSDLpPSLiquidity[1],
    };
    const psUSDCLpContract = new ethers.Contract(
      '0xabfa93b2b786b6ea3c8d990cc2efe81c880cb09f',
      winstonRKLPSABI,
      provider
    );
    const psUSDCLpPSLiquidity = await psUSDCLpContract.getReserves();
    const psUSDCLp = {
      reserve0: psUSDCLpPSLiquidity[0],
      reserve1: psUSDCLpPSLiquidity[1],
    };
    const psUSDTLpContract = new ethers.Contract(
      '0xe01d99478b8a159b957d75455c01104734e05ee1',
      winstonRKLPSABI,
      provider
    );
    const psUSDTLpPSLiquidity = await psUSDTLpContract.getReserves();
    const psUSDTLp = {
      reserve0: psUSDTLpPSLiquidity[0],
      reserve1: psUSDTLpPSLiquidity[1],
    };
    const psBTCBLpContract = new ethers.Contract(
      '0x656fb87cda1f25e8181b0415ef65dd6d439165e4',
      winstonRKLPSABI,
      provider
    );
    const psBTCBLpPSLiquidity = await psBTCBLpContract.getReserves();
    const psBTCBLp = {
      reserve0: psBTCBLpPSLiquidity[0],
      reserve1: psBTCBLpPSLiquidity[1],
    };
    const psCAKELpContract = new ethers.Contract(
      '0xb2a13ce9f3dfeb5f38f2211fbf3de91d72cdabdd',
      winstonRKLPSABI,
      provider
    );
    const psCAKELpPSLiquidity = await psCAKELpContract.getReserves();
    const psCAKELp = {
      reserve0: psCAKELpPSLiquidity[0],
      reserve1: psCAKELpPSLiquidity[1],
    };
    const yf1USELpContract = new ethers.Contract(
      '0x4c24b7a71e7576d6623f62bb83af63f31230ddfc',
      winstonRKLYFABI,
      provider
    );
    const yf1USELpPSLiquidity = await yf1USELpContract.getReserves();
    const yf1USELp = {
      reserve0: yf1USELpPSLiquidity[0],
      reserve1: yf1USELpPSLiquidity[1],
    };
    const psHCLpContract = new ethers.Contract(
      '0x8a7dc07d76b6d5f897a32a03731e8b5b06a656e7',
      winstonRKLPSABI,
      provider
    );
    const psHCLpPSLiquidity = await psHCLpContract.getReserves();
    const psHCLp = {
      reserve0: psHCLpPSLiquidity[0],
      reserve1: psHCLpPSLiquidity[1],
    };
    const yfWBNBLpContract = new ethers.Contract(
    ...
    const yfWBNBLp = {
      reserve0: yfWBNBLpPSLiquidity[0],
      reserve1: yfWBNBLpPSLiquidity[1],
    };
    const yfBUSDLpContract = new ethers.Contract(
      '0x7e8c3ad495970e4ce7c26a135f8e3b93cba726c5',
      winstonRKLYFABI,
      provider
    );
    const yfBUSDLpPSLiquidity = await yfBUSDLpContract.getReserves();
    const yfBUSDLp = {
      reserve0: yfBUSDLpPSLiquidity[0],
      reserve1: yfBUSDLpPSLiquidity[1],
    };
    const yfUSDCLpContract = new ethers.Contract(
      '0x9eb6a50dfddbfe710298d5139762301de8233d2c',
      winstonRKLYFABI,
      provider
    );
    const yfUSDCLpPSLiquidity = await yfUSDCLpContract.getReserves();
    const yfUSDCLp = {
      reserve0: yfUSDCLpPSLiquidity[0],
      reserve1: yfUSDCLpPSLiquidity[1],
    };
    const yfUSDTLpContract = new ethers.Contract(
      '0xb3b7766717169facf3eb53b9e1e2c4266f7945f8',
      winstonRKLYFABI,
      provider
    );
    const yfUSDTLpPSLiquidity = await yfUSDTLpContract.getReserves();
    const yfUSDTLp = {
      reserve0: yfUSDTLpPSLiquidity[0],
      reserve1: yfUSDTLpPSLiquidity[1],
    };
    const yfBTCBLpContract = new ethers.Contract(
      '0xc91878b2087fbe42c378b3667dde6ccb4d143392',
      winstonRKLYFABI,
      provider
    );
    const yfBTCBLpPSLiquidity = await yfBTCBLpContract.getReserves();
    const yfBTCBLp = {
      reserve0: yfBTCBLpPSLiquidity[0],
      reserve1: yfBTCBLpPSLiquidity[1],
    };
    const yfFIELDLpContract = new ethers.Contract(
      '0x6a451dc09f877781dd98cc31270adae256e21e09',
      winstonRKLYFABI,
      provider
    );
    const yfFIELDLpPSLiquidity = await yfFIELDLpContract.getReserves();
    const yfFIELDLp = {
      reserve0: yfFIELDLpPSLiquidity[0],
      reserve1: yfFIELDLpPSLiquidity[1],
    };
    const yfWMUELpContract = new ethers.Contract(
      '0x9b5ef226ff4b2b6ed395dac3de2786cdcd83a461',
      winstonRKLYFABI,
      provider
    );
    const yfWMUELpPSLiquidity = await yfWMUELpContract.getReserves();
    const yfWMUELp = {
      reserve0: yfWMUELpPSLiquidity[0],
      reserve1: yfWMUELpPSLiquidity[1],
    };
    const ssWBNBLpContract = new ethers.Contract(
      '0xb0d1a98c5213d60ab4932779eb695943fe1aeb51',
      winstonRKLYFABI,
      provider
    );
    const ssWBNBLpPSLiquidity = await ssWBNBLpContract.getReserves();
    const ssWBNBLp = {
      reserve0: ssWBNBLpPSLiquidity[0],
      reserve1: ssWBNBLpPSLiquidity[1],
    };
    const ssUSDTLpContract = new ethers.Contract(
      '0x71ffbe76c7e799a9e7bf8ffcd68e15ea4e175467',
      winstonRKLYFABI,
      provider
    );
    const ssUSDTLpPSLiquidity = await ssUSDTLpContract.getReserves();
    const ssUSDTLp = {
      reserve0: ssUSDTLpPSLiquidity[0],
      reserve1: ssUSDTLpPSLiquidity[1],
    };
  ...
  React.useEffect(async () => {
    if (!metamaskDownloadEnabled && !window.ethereum)
      enableDownloadMetamask(true);
  }, [metamaskDownloadEnabled, enableDownloadMetamask]);

  function LiquidityCell(props: any) {
    return (
      <TableRow>
        <TableCell>{props.exchange}</TableCell>
        <TableCell>{props.pool}</TableCell>
        <TableCell>{props.reserve0.toString() / 1e18}</TableCell>
        <TableCell>{props.reserve1.toString() / 1e18}</TableCell>
        <TableCell>{props.ratio.toString()}</TableCell>
        <TableCell>{props.price0.toString()}</TableCell>
        <TableCell>{props.price1.toString()}</TableCell>
      </TableRow>
    );
  }
  ...
  return (
    <Container>
      <Typography variant="h2" textAlign={'center'} mt={8} mb={3}>
        <img
          src="https://dropbox.winston.services/official-logos/Winston_Logo_W-White.png"
          width="300px"
        />
      </Typography>
          ...
          <Grid container spacing={{ xs: 2, md: 3 }}>
            <Grid item xs={12} sm={6} md={4}>
              <Card elevation={1} sx={{ py: 3, px: 2 }}>
                <CardHeader
                  avatar={
                    <Avatar src="https://dropbox.blockchange.services/rickle/Rickle-Front-full-color.png">
                      RKL
                    </Avatar>
                  }
                  title="Rickle"
                ...
                <Typography variant="h5" mt={1} textAlign={'center'}>
                  $
                  {(
                    (supplyLocker.rickleTotalSupply * coinPrices.RKL) /
                    1e18
                  ).toFixed(12)}
                </Typography>
              </Card>
            ...
            <Grid item xs={12} sm={6} md={4}>
              <Card elevation={1} sx={{ py: 3, px: 2 }}>
                ...
                <Typography variant="h5" mt={1} textAlign={'center'}>
                  $
                  {(
                    (supplyLocker.totalSupply *
                      (coinPrices.RKL / coinPrices.WIN)) /
                    1e18
                  ).toFixed(12)}
                </Typography>
              </Card>
          ...
          <Grid container spacing={{ xs: 2, md: 3 }}>
            ...
            <Grid item xs={12} sm={6} md={5}>
              <Card elevation={0} sx={{ py: 3, px: 2 }}>
                ...
                <List>
                  <ListItem>Locked Supply</ListItem>
                  <ListItem>
                  ...
                  <ListItem>Team Wallet</ListItem>
                  <ListItem>
                    <small>0x130Be432f446F1fD0B356023f3b308C36a304642</small>
                  </ListItem>
                  <ListItem>Funding Wallet</ListItem>
                  <ListItem style={{ fontSize: 'smaller' }}>
                    <small>{FundraiserWallet}</small>
                  </ListItem>
                </List>
                <Typography variant="h5" mt={1} textAlign={'center'}>
                  <a href="https://dicord.gg/rickletoken">Discord</a> |{' '}
                  <a href="https://dropbox.winston.services/documentation/Winston%20Design.pdf">
                    Whitepaper
                  </a>
                </Typography>
              </Card>
          ...
          <Grid container display="flex" justifyContent={'center'}>
            <Grid item xs={12} sm={8} md={6} lg={5}>
              ...
              <Grid mt={3}>
                ...
                {isPriceImpactToHigh ? (
                  <Typography variant="h5">Price impact too High.</Typography>
                ) : (
                  <Typography variant="h5">Max Purchase 6000 WIN</Typography>
                )}
              </Grid>
              <Button
                fullWidth
            ...
            <TableBody>
              <TableRow>
                ...
                <TableCell>
                  $
                  {(
                    supplyLocker.bnbHeldInGasStation * coinPrices['WBNB']
                  ).toString() / 1e18}
                </TableCell>
              </TableRow>
              ...
              <TableRow>
                ...
                <TableCell>
                  $
                  {(
                    supplyLocker.winstonHeldInGasStation *
                    (coinPrices['RKL'] / coinPrices['WIN'])
                  ).toString() / 1e18}
                </TableCell>
              </TableRow>
            ...
            <TableBody>
              ...
              <TableRow>
                ...
                <TableCell>
                  $
                  {(
                    supplyLocker.fieldHeldInFundraiserWallet *
                    coinPrices['FIELD']
                  ).toString() / 1e18}
                </TableCell>
              </TableRow>
            ...
            <TableBody>
              {[
                ...
                },
                {
                  ...
                  ratio:
                    supplyLocker.psRKLLp.reserve0 /
                    supplyLocker.psRKLLp.reserve1,
                  price0:
                    (supplyLocker.psRKLLp.reserve0 *
                      (coinPrices.RKL / coinPrices.WIN)) /
                    1e18,
                  price1:
                    (supplyLocker.psRKLLp.reserve1 * coinPrices.RKL) / 1e18,
                },
                {
                  exchange: 'YF',
                  pool: 'WIN/RKL',
                  ...
                  ratio:
                    supplyLocker.yfRKLLp.reserve0 /
                    supplyLocker.yfRKLLp.reserve1,
                  price0:
                    (supplyLocker.yfRKLLp.reserve0 *
                      (coinPrices.RKL / coinPrices.WIN)) /
                    1e18,
                  price1:
                    (supplyLocker.yfRKLLp.reserve1 * coinPrices.RKL) / 1e18,
                },
                {
                  exchange: 'PS',
                  pool: 'WBNB/RKL',
                  ...
                  ratio:
                    supplyLocker.psWBNBLp.reserve0 /
                    supplyLocker.psWBNBLp.reserve1,
                  price0:
                    supplyLocker.psWBNBLp.reserve0 * (coinPrices.WBNB / 1e18),
                  price1:
                    (supplyLocker.psWBNBLp.reserve1 * coinPrices.RKL) / 1e18,
                },
                {
                  exchange: 'PS',
                  pool: 'BUSD/RKL',
                  ...
                  ratio:
                    supplyLocker.psBUSDLp.reserve0 /
                    supplyLocker.psBUSDLp.reserve1,
                  price0:
                    (supplyLocker.psBUSDLp.reserve0 * coinPrices.BUSD) / 1e18,
                  price1:
                    (supplyLocker.psBUSDLp.reserve1 * coinPrices.RKL) / 1e18,
                },
                {
                  exchange: 'PS',
                  pool: 'USDC/RKL',
                  ...
                  ratio:
                    supplyLocker.psUSDCLp.reserve0 /
                    supplyLocker.psUSDCLp.reserve1,
                  price0:
                    (supplyLocker.psUSDCLp.reserve0 * coinPrices.USDC) / 1e18,
                  price1:
                    (supplyLocker.psUSDCLp.reserve1 * coinPrices.RKL) / 1e18,
                },
                {
                  exchange: 'PS',
                  pool: 'USDT/RKL',
                  ...
                  ratio:
                    supplyLocker.psUSDTLp.reserve0 /
                    supplyLocker.psUSDTLp.reserve1,
                  price0:
                    (supplyLocker.psUSDTLp.reserve0 * coinPrices.USDT) / 1e18,
                  price1:
                    (supplyLocker.psUSDTLp.reserve1 * coinPrices.RKL) / 1e18,
                },
                {
                  exchange: 'PS',
                  pool: 'BTCB/RKL',
                  ...
                  ratio:
                    supplyLocker.psBTCBLp.reserve0 /
                    supplyLocker.psBTCBLp.reserve1,
                  price0: 0,
                  price1:
                    (supplyLocker.psBTCBLp.reserve1 * coinPrices.RKL) / 1e18,
                },
                {
                  exchange: 'PS',
                  pool: 'CAKE/RKL',
                  ...
                  ratio:
                    supplyLocker.psCAKELp.reserve0 /
                    supplyLocker.psCAKELp.reserve1,
                  price0: 0,
                  price1:
                    (supplyLocker.psCAKELp.reserve1 * coinPrices.RKL) / 1e18,
                },
                /*{
                exchange: "PS",
                pool: "BHT/RKL",
                ...
                price1:0
                  ...
                  ratio:
                    supplyLocker.psHCLp.reserve0 / supplyLocker.psHCLp.reserve1,
                  price0: 0,
                  price1: 0,
                },
                {
                  ...
                  ratio:
                    supplyLocker.yfWBNBLp.reserve0 /
                    supplyLocker.yfWBNBLp.reserve1,
                  price0:
                    supplyLocker.yfWBNBLp.reserve0 * (coinPrices.WBNB / 1e18),
                  price1:
                    (supplyLocker.yfWBNBLp.reserve1 * coinPrices.RKL) / 1e18,
                },
                {
                  exchange: 'YF',
                  pool: 'BUSD/RKL',
                  ...
                  ratio:
                    supplyLocker.yfBUSDLp.reserve0 /
                    supplyLocker.yfBUSDLp.reserve1,
                  price0:
                    (supplyLocker.yfBUSDLp.reserve0 * coinPrices.BUSD) / 1e18,
                  price1:
                    (supplyLocker.yfBUSDLp.reserve1 * coinPrices.RKL) / 1e18,
                },
                {
                  exchange: 'YF',
                  pool: 'USDC/RKL',
                  ...
                  ratio:
                    supplyLocker.yfUSDCLp.reserve0 /
                    supplyLocker.yfUSDCLp.reserve1,
                  price0:
                    (supplyLocker.yfUSDCLp.reserve0 * coinPrices.USDC) / 1e18,
                  price1:
                    (supplyLocker.yfUSDCLp.reserve1 * coinPrices.RKL) / 1e18,
                },
                {
                  exchange: 'YF',
                  pool: 'USDT/RKL',
                  ...
                  ratio:
                    supplyLocker.yfUSDTLp.reserve0 /
                    supplyLocker.yfUSDTLp.reserve1,
                  price0:
                    (supplyLocker.yfUSDTLp.reserve0 * coinPrices.USDT) / 1e18,
                  price1:
                    (supplyLocker.yfUSDTLp.reserve1 * coinPrices.RKL) / 1e18,
                },
                {
                  exchange: 'YF',
                  pool: 'BTCB/RKL',
                  ...
                  ratio:
                    supplyLocker.yfBTCBLp.reserve0 /
                    supplyLocker.yfBTCBLp.reserve1,
                  price0: 0,
                  price1:
                    (supplyLocker.yfBTCBLp.reserve1 * coinPrices.RKL) / 1e18,
                },
                {
                  exchange: 'YF',
                  pool: 'FIELD/RKL',
                  ...
                  ratio:
                    supplyLocker.yfFIELDLp.reserve0 /
                    supplyLocker.yfFIELDLp.reserve1,
                  price0:
                    (supplyLocker.yfFIELDLp.reserve0 * coinPrices.FIELD) / 1e18,
                  price1:
                    (supplyLocker.yfFIELDLp.reserve1 * coinPrices.RKL) / 1e18,
                },
                {
                  exchange: 'YF',
                  pool: 'WMUE/RKL',
                  ...
                  ratio:
                    supplyLocker.yfWMUELp.reserve0 /
                    supplyLocker.yfWMUELp.reserve1,
                  price0:
                    (supplyLocker.yfWMUELp.reserve0 * coinPrices.WMUE) / 1e18,
                  price1:
                    (supplyLocker.yfWMUELp.reserve1 * coinPrices.RKL) / 1e18,
                },
                {
                  exchange: 'YF',
                  pool: '1USE/RKL',
                  ...
                  ratio:
                    supplyLocker.yf1USELp.reserve0 /
                    supplyLocker.yf1USELp.reserve1,
                  price0: 0,
                  price1:
                    (supplyLocker.yf1USELp.reserve1 * coinPrices.RKL) / 1e18,
                },
                {
                  exchange: 'SS',
                  pool: 'WBNB/RKL',
                  ...
                  ratio:
                    supplyLocker.ssWBNBLp.reserve0 /
                    supplyLocker.ssWBNBLp.reserve1,
                  price0:
                    supplyLocker.ssWBNBLp.reserve0 * (coinPrices.WBNB / 1e18),
                  price1:
                    (supplyLocker.ssWBNBLp.reserve1 * coinPrices.RKL) / 1e18,
                },
                {
                  exchange: 'SS',
                  pool: 'USDT/RKL',
                  ...
                  ratio:
                    supplyLocker.ssUSDTLp.reserve0 /
                    supplyLocker.ssUSDTLp.reserve1,
                  price0:
                    (supplyLocker.ssUSDTLp.reserve0 * coinPrices.USDT) / 1e18,
                  price1:
                    (supplyLocker.ssUSDTLp.reserve1 * coinPrices.RKL) / 1e18,
                },
                {
                  exchange: 'CS',
                  pool: 'WBNB/RKL',
